...
- **Theme registry** (`registry.ts`): `getAllThemes()`, `findTheme()`, `getActiveThemeId()`, `setActiveThemeId()`, `getCustomThemes()`, `setCustomThemes()`, `applyThemeToDocument()`. Storage keys `t3code:active-theme` and `t3code:custom-themes`.
