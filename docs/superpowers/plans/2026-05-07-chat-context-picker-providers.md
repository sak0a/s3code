...
- Use the existing `truncateSourceControlDetailContent` helper from `@ryco/contracts` for body+comments truncation. Don't reinvent.
...
```bash
cd /Users/laurinfrank/Library/CloudStorage/Dropbox/Code/ryco && bun typecheck
```
...
> **Note:** `BitbucketPullRequestSchema` and `normalizeBitbucketPullRequestRecord` are already in scope in `bitbucketPullRequests.ts` from prior code; reuse them. Add `import { decodeJsonResult } from "@ryco/shared/schemaJson";` if not already imported (search the file first; do not duplicate). The decoder may need `Cause`, `Result`, `Schema` imports — check the existing import block.
...
```ts
import { Cause, Exit, Option, Result, Schema } from "effect";
import { PositiveInt } from "@ryco/contracts";
import { decodeJsonResult, formatSchemaError } from "@ryco/shared/schemaJson";
...
- [ ] In a **GitLab** workspace with `glab` installed and authed: open Ryco → click 📎 → see GL issues → attach one → send → verify the agent received the structured context. Same for an MR.
- [ ] In a **Bitbucket** workspace with `RYCO_BITBUCKET_*` env vars configured: same flow.
