...
with the same server-side behavior as `s3code serve --tailscale-serve`, then the server asks Tailscale
...
Use `s3code serve --help` for the full flag reference. It supports the same general startup options as the normal server command, including an optional `cwd` argument.
...
> For now, use `s3code project ...` on the server machine instead.
...
1. `s3code serve` issues a one-time owner pairing token.
...
Use `s3code auth` to manage access after the initial pairing flow.
...
- Use `s3code auth` to revoke credentials or sessions you no longer trust.
